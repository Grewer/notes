...
我想前端从业人员或多或少会听说过这个词，但是他具体是什么， 怎么入门， 怎么学习是一个较高的门槛，本文就 V8 入门，来做一个记录，也方便大家的入门。
...
为了提高性能，引入了 Java 虚拟机和 C++ 编译器中的众多技术。现在 JavaScript 引擎的执行过程大致是：
...
JavaScript 代码编译的过程大致为：

`Script` 类调用 `Compiler` 类的 `Compile` 函数为其生成本地代码。
`Compile` 函数先使用 `Parser` 类生成 AST，再使用 `FullCodeGenerator` 类来生成本地代码。
