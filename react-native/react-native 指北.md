...
之前写的一篇文章有详细记载 APP 的更新, 这里先附上他的手动搭建原理:
...
原有的架构我在上文中已经讲过了, 他存在一些问题:
目前 RN 使用 Bridge Module通信(其中还需要数据转换和解码), 发送的消息本质上是异步的, 也就是说如果是即时性比较高的操作, 比如拖拽, 就会出现失帧的情况
...
举个简单的例子就是, 父组件调用子组件方法的场景

原来是在父组件传递给子组件消息方法, 之后子组件中收到消息并执行

而现在是通过 `ref`, 直接就调用了方法了` ref.current.fn()`
